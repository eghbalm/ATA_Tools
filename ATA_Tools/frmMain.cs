...
namespace ATA_Tools
{
    public unsafe partial class frmMain : Form
    {
        ...
        private void Monitorings()
        {
            ...
            //UInt32 Status;
            //Error = 0;
            //p.GetPortValue((UInt16)(baseAddress  + 1), &Error, 1);
            //Status = 0;
            //p.GetPortValue((UInt16)(baseAddress + 7), &Status, 1);
            ...
            uint Status;
            p.GetPortValue((UInt16)(baseAddress + 1), &Error, 1);
            mon1.Text = getNormalCount(Error.ToString("x"));
            ...
            p.GetPortValue((UInt16)(baseAddress + 7), &Status, 1);
            mon7.Text = getNormalCount(Status.ToString("x"));

            p.rError.setErrorRegister(Error);
            p.rStatus.setStatusRegister(Status);

            if (p.rStatus.ERR) lbEror.BackColor = Color.Red; else lbEror.BackColor = Color.DarkGray;
            if (p.rStatus.IDX) lbIdx.BackColor = Color.Green; else lbIdx.BackColor = Color.DarkGray;
            if (p.rStatus.CORR) lbCorr.BackColor = Color.Green; else lbCorr.BackColor = Color.DarkGray;
            if (p.rStatus.DRQ) lbDrq.BackColor = Color.Green; else lbDrq.BackColor = Color.DarkGray;
            if (p.rStatus.DSC) lbDsc.BackColor = Color.Green; else lbDsc.BackColor = Color.DarkGray;
            if (p.rStatus.DF) lbDf.BackColor = Color.Red; else lbDf.BackColor = Color.DarkGray;
            if (p.rStatus.DRDY) lbDrdy.BackColor = Color.Green; else lbDrdy.BackColor = Color.DarkGray;
            if (p.rStatus.BSY) lbBsy.BackColor = Color.Green; else lbBsy.BackColor = Color.DarkGray;
            ////////////////////////////////////////////////////////////
            if (p.rError.BBK) lbBBK.BackColor = Color.Red; else lbBBK.BackColor = Color.DarkGray;
            if (p.rError.UNC) lbUNC.BackColor = Color.Red; else lbUNC.BackColor = Color.DarkGray;
            if (p.rError.MC) lbMC.BackColor = Color.Red; else lbMC.BackColor = Color.DarkGray;
            if (p.rError.IDNF) lbIDNF.BackColor = Color.Red; else lbIDNF.BackColor = Color.DarkGray;
            if (p.rError.MCR) lbMCR.BackColor = Color.Red; else lbMCR.BackColor = Color.DarkGray;
            if (p.rError.ABRT) lbABRT.BackColor = Color.Red; else lbABRT.BackColor = Color.DarkGray;
            if (p.rError.TONF) lbTONF.BackColor = Color.Red; else lbTONF.BackColor = Color.DarkGray;
            if (p.rError.AMNF) lbAMNF.BackColor = Color.Red; else lbAMNF.BackColor = Color.DarkGray;            
        }
                                ...
                                for (int i2 = loopir[0].SectorCount; i2 < loopir[1].SectorCount + 1; i2++)
                                {
                                    ...
                                    {                                        
                                        ...
                                        while (--waitLoop > 0)
                                        {
                                            ...
                                            //  previous drive command ended in error
                                            if (portValue == 0x51) {  lbDrdy.BackColor = Color.Green; lbDsc.BackColor = Color.Green; lbEror.BackColor = Color.Red; break; }
                                            //  drive is ready and drq
                                            if (portValue == 0x58)
                                            {
                                                if (ifDrq.Checked)
        ...
        {
            UInt32 portValue;
            p.GetPortValue((UInt16)(baseAddress + 0x0C), &portValue, 1);
            p.GetPortValue((UInt16)(baseAddress + 0x0C), &portValue, 1);
            p.GetPortValue((UInt16)(baseAddress + 0x0C), &portValue, 1);
            p.GetPortValue((UInt16)(baseAddress + 0x0C), &portValue, 1);
        }
        ...
        {
            uint BaseController = (UInt16)(baseAddress - 16);
            p.SetPortValue((UInt16)(BaseController + 2), 0x04, 1);
            ioWait();
            p.SetPortValue((UInt16)(BaseController + 2), 0x00, 1);
            p.SetPortValue((UInt16)(baseAddress + 7), 0x08, 1);
        }
